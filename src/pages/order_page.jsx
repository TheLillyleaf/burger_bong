...
export default function OrderPage() {
  ...
  const can_order = name.trim().length > 0 && selected_burger !== null && !submitting
  ...
  async function handle_submit(e) {
    ...
    try {
      ...
      set_confirmation({ name: name.trim(), burger: burger_data?.name ?? selected_burger })
  ...
  return (
    <div className="order-page">
      <header className="order-page__header">
        <div className="order-page__logo">Burger Bong 🍔</div>
        <div className="order-page__subtitle">Välj din burgare och beställ</div>
      </header>

      <form className="order-page__body" onSubmit={handle_submit}>
        <OrderForm value={name} on_change={set_name} />
        ...
        <button type="submit" className="order-btn" disabled={!can_order}>
